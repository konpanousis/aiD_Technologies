...
class DataWriter():
    def __init__(self, cfg, opt, save_video=False,
                 ...
                 queueSize=1024, text = 'Unknown', signer = 'signer01', gloss = ''):
        self.cfg = cfg
        self.opt = opt
    ...
    def update(self):
        ...
        while True:
            ...
            if orig_img is None:
                ...
                if self.save_video:
                    stream.release()
                write_json(final_result, self.opt.outputpath, form=self.opt.format, for_eval=self.opt.eval, text = self.txt, signer = self.signer, gloss = self.gloss,
                           outputfile = self.video_save_opt['savepath'].split('/')[-1][:-4] +'.json')
